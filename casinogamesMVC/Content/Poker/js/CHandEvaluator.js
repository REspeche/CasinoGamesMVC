function CHandEvaluator(){
    ...
    this._isFlush = function(){
        if(    (_aSortedHand[4].suit === _aSortedHand[0].suit || _aSortedHand[0].rank === CARD_JOKER)
            && (_aSortedHand[4].suit === _aSortedHand[1].suit || _aSortedHand[1].rank === CARD_JOKER)
            && (_aSortedHand[4].suit === _aSortedHand[2].suit || _aSortedHand[2].rank === CARD_JOKER)
            && (_aSortedHand[4].suit === _aSortedHand[3].suit || _aSortedHand[3].rank === CARD_JOKER)){
            return true;
        }else{
            return false;
        }
    ...
    this._isStraight = function() {
        var iNumDeuces = this.isJokerInHand()?1:0;

        var iUsedDeuces = 0;
        for(var i=0;i<_aSortedHand.length-1;i++){
